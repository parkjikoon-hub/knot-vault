...
- Codex CLI의 경우 `--codex` 플래그로 설치하면 자동으로 참조됨
...
**목적**: Codex CLI가 Obsidian 고유 문법을 정확히 사용하여 노트를 생성/편집하게 한다.
...
**목적**: Codex CLI가 실행 중인 Obsidian 앱을 터미널 명령어로 직접 조작할 수 있게 한다.
...
**목적**: Codex CLI가 `.base` 파일(Obsidian의 내장 데이터베이스 뷰 기능)을 정확하게 생성/편집하게 한다.
...
**목적**: Codex CLI가 Obsidian Canvas 파일을 코드로 생성하고 편집할 수 있게 한다.
...
# Codex CLI 스킬 폴더로 복사
cp -r kepano-obsidian-skills/skills/* .codex/skills/
```

### 방법 3: Claude Code 마켓플레이스 (Claude Code가 있는 경우)
...
*이 문서는 2026-05-04 기준 kepano/obsidian-skills v1.0.1을 분석하여 작성되었음. Codex CLI (Windows) — general 폴더.*
